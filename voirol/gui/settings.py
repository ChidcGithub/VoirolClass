...
class SettingsDialog(QDialog):
    ...
    def _add_model_tab(self):
        ...
        hf_label = QLabel(t("model.hf_mirror_url"))
        layout.addWidget(hf_label)

        hf_layout = QHBoxLayout()
        self._hf_mirror_input = QLineEdit()
    ...
    def _add_agent_tab(self):
        ...
        hint_label = QLabel(t("agent.restart_hint"))
    ...
    def _add_tts_tab(self):
        ...
        self._tts_enabled_cb = QCheckBox(t("tts.enable"))
        self._tts_enabled_cb.setChecked(tts_cfg.get("enabled", False))
        self._tts_enabled_cb.toggled.connect(self._on_tts_config_changed)
        layout.addWidget(self._tts_enabled_cb)

        voice_label = QLabel(t("tts.voice"))
        ...
        model_label = QLabel(t("tts.model_path"))
        layout.addWidget(model_label)
        ...
        tok_label = QLabel(t("tts.audio_tokenizer_path"))
        layout.addWidget(tok_label)

        self._tts_tok_path = QLineEdit()
        ...
        mirror_layout = QHBoxLayout()
        self._tts_mirror_input = QLineEdit()
        ...
        hf_layout = QHBoxLayout()
        self._tts_hf_mirror_input = QLineEdit()
        ...
        self._tts_py_btn = QPushButton(t("tts.dl_python"))
        ...
        self._tts_dl_ready_label.setStyleSheet("color: #4caf50; font-weight: bold; font-size: 12px;")
        layout.addWidget(self._tts_dl_ready_label)

        self._tts_progress_bar = QProgressBar()
        self._tts_progress_bar.setMinimum(0)
        self._tts_progress_bar.setMaximum(100)
        ...
        self._tts_status_label.setStyleSheet("color: #888; font-size: 11px;")
        ...
        layout.addStretch()
        self.tabs.addTab(tab, t("tts.tab"))
    ...
    def _tts_pip_install_worker(self, python_path: str):
        import subprocess, os, io, zipfile, tempfile, shutil, requests
    ...
    def _on_tts_progress(self, pct: int):
        if pct == -1:
            ...
            self._tts_progress_bar.setValue(pct)
    ...
    def _add_log_tab(self):
        ...
        from voirol.utils.logger import _log_signal, _global_log_buffer

        # Pre-populate from global buffer (all logs since process start)
